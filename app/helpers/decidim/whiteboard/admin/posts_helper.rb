      ...
      module PostsHelper
        ...
        def publish_data(published_at)
          ...
          if published_at > Time.current
            data[:icon] = icon("time-line", aria_label: t("decidim.whiteboard.admin.posts.index.not_published_yet"), role: "img")
            data[:popup] = t("decidim.whiteboard.admin.posts.index.not_published_yet")
          end
          data
        end
      end
